...
export const favoritesApi = {
  ...
  getUserFavorites: async (userId: string): Promise<FavoriteFood[]> => {
    const response = await apiClient.get<FavoriteFood[]>(`/users/${userId}/favorites`);
    return response.data;
  },

  // Add a food to favorites
  addFavorite: async (userId: string, foodId: string): Promise<FavoriteFood> => {
    const response = await apiClient.post<FavoriteFood>(`/users/${userId}/favorites`, { food_id: foodId });
    return response.data;
  },
